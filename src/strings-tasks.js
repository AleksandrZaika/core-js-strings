...
function isPalindrome(str) {
  const strLowerCase = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
  const reverse = strLowerCase.split('').reverse().join('');
  return reverse === strLowerCase;
}
...
function extractNameFromTemplate(value) {
  const splitValue = value.split(/Hello, |!/);
  return splitValue[1];
}
...
function extractEmails(str) {
  const emailsList = str.split(';');
  const filteredEmails = emailsList.map((email) => email.trim());
